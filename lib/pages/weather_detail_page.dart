...
class _WeatherDetailPageState extends State<WeatherDetailPage> {
  ...
  final WeatherService _weatherService =
      WeatherService(); // Instantiate WeatherService
  ...
  final ScrollController _hourlyScrollController =
      ScrollController(); // Add ScrollController
  final ScrollController _dayScrollController =
      ScrollController(); // Add ScrollController for day tabs
  ...
  void didChangeDependencies() {
    ...
    if (locationArg != null && locationArg is String) {
      ...
      String coordinates =
          "${locationArg.split(',')[0]},${locationArg.split(',')[1]}";
...
void _updateSelectedDayOnScroll() {
  ...
  void _scrollToDayTab(int index) {
    ...
    final RenderBox listViewRenderBox =
        _dayScrollController.position.context.storageContext.findRenderObject()
            as RenderBox;

    final double widgetStart =
        dayRenderBox.localToGlobal(Offset.zero, ancestor: listViewRenderBox).dx;
  ...
  String _getDayDisplay(String dateString, int index) {
    ...
    if (index < 7) {
      return DateFormat(
        'EEE',
      ).format(date); // Mon, Tue, etc. for the first week
    } else {
      return DateFormat(
        'MMM d',
      ).format(date); // Jan 1, Feb 15, etc. for subsequent weeks
    }
  }
  ...
  Map<String, String> _getSunTimes() {
    ...
    DateTime parseTime(String timeStr) {
      ...
      return DateTime.parse(
        '$date ${hour.toString().padLeft(2, '0')}:${minutes.toString().padLeft(2, '0')}:00',
      );
    }
                    ...
                    if (_weatherData!['forecast'] != null)
                      SizedBox(
                        ...
                        child: ListView.builder(
                          ...
                          itemCount:
                              (_weatherData!['forecast']['forecastday'] as List)
                                  .length,
                          itemBuilder: (context, index) {
                            ...
                            final dayDisplay = _getDayDisplay(
                              dateString,
                              index,
                            );
                            ...
                            return GestureDetector(
                              ...
                              onTap: () {
                                ...
                                final startIndex = _getHourlyStartIndexForDay(
                                  index,
                                );
                                _hourlyScrollController.animateTo(
                                  startIndex *
                                      108.0, // Item width: 100 + 4*2 padding/margin
                                  duration: const Duration(milliseconds: 300),
                                  curve: Curves.easeOut,
                                );
